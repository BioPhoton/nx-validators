...
const createValidatorDocumentationUrl = (validatorId: string): string =>
    `https://vie.git.bwinparty.com/vanilla/vanilla/-/tree/master/Client/migration-kit/src/workspace-validation/validators/${validatorId}/README.md`;

const createValidationResult = (acc, [validationId, { name, description, validatorIds }]): WorkspaceValidationResult['validationResults'] => {
    return {
        ...acc,
        [validationId]: {
            name,
...
export const resultStatusPriorityOrder: ResultStatus[] = ['skip', 'success', 'failed'];
export function updateValidatorResult(
    ...
    const currentValidationStatus = workspaceResult.validationResults[validationId].status;
    const validatorStatus = validatorResult.status;
    const newValidationStatus =
        resultStatusPriorityOrder.indexOf(validatorStatus) > resultStatusPriorityOrder.indexOf(currentValidationStatus)
            ? validatorStatus
